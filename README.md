...
The elasticity of subsidies with respect to the average monomial price is $\eta_{p_m}^s = p_m/(p_m - p_e)$. Given that throughout the entire period considered, p௠ > p௘, the average monomial price is always elastic with respect to energy subsidies. Moreover, it is observed that this variable is the most elastic throughout the studied period.
